...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (s,) = ctx.saved_values
        s2 = s.f.mul_zip(s, s)
        inner = s.f.add_zip(s, s.f.neg_map(s2))
        return grad_output.f.mul_zip(grad_output, inner)
...
class Log(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        return grad_output * t1.f.inv_map(t1)
...
class Permute(Function):
    @staticmethod
    def forward(ctx: Context, a: Tensor, order1: Tensor) -> Tensor:
        raw = order1._tensor._storage  # 假设底层是 list 或 array
        order = [int(x) for x in raw]
        assert len(order) == len(a.shape), "Permutation length must match tensor ndim."
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        for i, ax in enumerate(order):
            inv[ax] = i
        new_td = grad_output._tensor.permute(*inv)
        grad_input = grad_output._new(new_td)
